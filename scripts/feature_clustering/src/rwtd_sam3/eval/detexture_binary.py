"""DeTexture ADE20K adapter for registered cross-dataset SAM-feature experiments.

This module exposes the local DeTexture ADE20K crop/mask assets behind
``predict-detexture-binary`` and ``eval-detexture-binary``. It reuses the same
registered SAM-feature experiments used on RWTD, STLD, and CAID, applies
permutation-invariant binary scoring, and writes the standard run artifacts
under ``outputs/detexture_binary/``.
"""
...
from rwtd_sam3.data.detexture_binary import (
    DETEXTURE_DATASET_ID,
    DeTextureBinaryOverview,
    DeTextureBinarySample,
    get_detexture_binary_sample,
    iter_detexture_binary_samples,
    load_detexture_binary_overview,
)
...
DETEXTURE_BINARY_DEFAULT_VARIANT = DEFAULT_CROSS_DATASET_EXPERIMENT
DETEXTURE_BINARY_SUPPORTED_VARIANTS = CROSS_DATASET_EXPERIMENT_VARIANTS
DETEXTURE_BINARY_OUTPUT_ROOT = Path("outputs") / "detexture_binary"
DETEXTURE_HARDWARE_COMPATIBILITY_STANDARD = "detexture_streaming_eval_v1"
DETEXTURE_HARDWARE_COMPATIBILITY_DESCRIPTION = (
    "DeTexture evaluation must stream decoded crop/mask triples instead of preloading the dataset, "
    "must resolve the asset root once per run, and must persist the active sample-loading mode into run artifacts."
)
DETEXTURE_BINARY_SCALAR_FIELDS = (
    "eval_miou",
    "eval_ari",
    "miou",
    "ari",
...
class DeTextureBinarySampleResult:
    """Per-sample DeTexture ADE20K benchmark payload."""
...
def run_detexture_binary_predict_one(args) -> dict[str, Any]:
    """Run one registered experiment on one DeTexture ADE20K sample."""

    sample = get_detexture_binary_sample(dataset_root=args.dataset_root, index=args.index)
    output_dir = prepare_detexture_binary_output_dir(
        args.output_dir,
    ...
    overview = load_detexture_binary_overview(args.dataset_root)
    config_payload = build_detexture_binary_run_config(
        args,
        overview=overview,
    ...
    write_text(
        ...
        build_detexture_binary_experiment_terms_markdown(
            args,
            dataset_partition=None,
            selected_sample_count=1,
        ),
    ...
    evaluation = evaluate_detexture_binary_sample(
        sample=sample,
        variant=args.variant,
        runner=runner,
    )
    ...
    if args.save_visuals:
        panel_path = save_detexture_binary_panel(
        ...
        write_jsonl(
            ...
            [
                build_detexture_binary_visual_record(
                    sample=sample,
                    evaluation=evaluation,
                    variant=args.variant,
...
def run_detexture_binary_evaluation(args) -> dict[str, Any]:
    """Run one registered experiment over the local DeTexture ADE20K assets."""
    ...
    if num_total_samples < 1:
        raise RuntimeError("The DeTexture ADE20K root did not yield any samples to evaluate.")
    ...
    LOGGER.info(
        "DeTexture eval uses streamed sample decoding under hardware standard %s; limit=%s.",
        DETEXTURE_HARDWARE_COMPATIBILITY_STANDARD,
        args.limit,
    )
    ...
    try:
        progress = tqdm(total=num_total_samples, desc="eval:detexture", unit="sample")
        for step, sample in enumerate(
            ...
            if args.wandb:
                ...
                if step % args.log_every == 0:
                    preview_image = render_detexture_binary_panel(
                        sample=sample,
                        evaluation=evaluation,
                        variant=args.variant,
                    )
                    caption = build_visual_caption(
                        sample=sample,
                        protocol=_detexture_protocol_label(args.variant),
        ...
        if not rows:
            raise RuntimeError(
                "The DeTexture ADE20K benchmark produced no successful evaluations. "
        ...
        summary = build_detexture_binary_summary(
            overview=overview,
            variant=args.variant,
            model_id=args.model_id,
            rows=rows,
        ...
        write_text(output_dir / "summary.md", build_detexture_binary_markdown_summary(summary))
    finally:
...
def evaluate_detexture_binary_sample(
    sample: DeTextureBinarySample,
    variant: str,
    runner,
) -> DeTextureBinarySampleResult:
    """Evaluate one registered experiment on one DeTexture ADE20K sample."""
    ...
    if variant == "boundary_refine_sweep":
        ...
        return DeTextureBinarySampleResult(
            row=row,
...
def build_detexture_binary_run_config(
    args,
    overview: DeTextureBinaryOverview | None,
    dataset_partition: DatasetPartitionSelection | None,
    selected_sample_count: int | None,
) -> dict[str, Any]:
    """Build a stable run-config payload for DeTexture ADE20K."""
    ...
    config = {
        ...
        "model_id": args.model_id,
        "device": args.device,
        "hardware_compatibility_standard": DETEXTURE_HARDWARE_COMPATIBILITY_STANDARD,
        "hardware_compatibility_description": DETEXTURE_HARDWARE_COMPATIBILITY_DESCRIPTION,
        ...
        "primary_metric_reason": (
            "The DeTexture ADE20K crops expose paired mask_a/mask_b supervision, so this run scores both "
            "cluster-to-mask assignments and reports the better partition-invariant result."
        ),
        "versions": discovered_package_versions(),
    }
    if overview is not None:
        config["resolved_dataset_root"] = str(overview.dataset_root)
        config["resolved_assets_root"] = str(overview.assets_root)
...
def build_detexture_binary_experiment_terms_markdown(
    args,
    *,
    dataset_partition: DatasetPartitionSelection | None,
    selected_sample_count: int | None,
) -> str:
    """Render a route-specific protocol-spec document for DeTexture ADE20K."""

    spec = get_cross_dataset_experiment_spec(args.variant)
    lines = [
        "# DeTexture ADE20K Benchmark Terms",
        "",
        "## Run Scope",
        "",
        f"- Command family: `{getattr(args, 'command', 'detexture-binary')}`",
        f"- Dataset id: `{DETEXTURE_DATASET_ID}`",
        f"- Requested dataset root: `{args.dataset_root}`",
        "- Expected local root form: `<root>/assets/crops` and `<root>/assets/masks` or the dataset root containing them.",
        ...
        f"- Hardware compatibility standard: `{DETEXTURE_HARDWARE_COMPATIBILITY_STANDARD}`",
        ...
        "## Dataset Semantics",
        "",
        "- Each sample is one crop image from `assets/crops` plus two paired masks from `assets/masks` named `<crop_stem>_mask_a.png` and `<crop_stem>_mask_b.png`.",
        "- The adapter treats the task as a two-region binary partition with permutation-invariant evaluation.",
        "- Cross-dataset experiment contract: every current-method experiment promoted into this adapter must register itself in `src/rwtd_sam3/eval/experiment_registry.py`, declare support for `rwtd`, `stld`, `caid`, and `detexture_ade20k`, and be documented in the root README before merge.",
        "",
        "## Protocol Standard",
        "",
        f"1. Run `{args.variant}` exactly as registered in the cross-dataset experiment registry.",
        "2. The dataset adapter reuses the registered runner unchanged and applies only DeTexture-specific GT decoding, permutation-invariant assignment, and output-directory handling.",
        ...
        "- Both cluster-to-mask assignments are scored and the better one is reported.",
        ...
        "## Output Contract",
        "",
        "- `prediction.json` / `prediction.png` for `predict-detexture-binary`.",
        "- `per_sample_metrics.csv`, `summary.json`, `summary.md`, and `visuals_manifest.jsonl` for `eval-detexture-binary`.",
        f"- The preview contract comes from the registry entry for `{args.variant}` and is currently `{spec.visual_contract}`.",
        "",
        "## Failure Semantics",
        "",
        "- There is no silent fallback to another dataset, route, or refinement stage.",
        "- Missing dataset roots, unmatched image/mask pairs, simultaneously empty paired masks, shape mismatches, or missing official Meta `sam3` features are explicit failures.",
...
def build_detexture_binary_summary(
    ...
    """Aggregate one local DeTexture ADE20K run."""

    spec = get_cross_dataset_experiment_spec(variant)
    mean_metrics = {name: mean(float(row[name]) for row in rows) for name in DETEXTURE_BINARY_SCALAR_FIELDS}
    median_metrics = {name: median(float(row[name]) for row in rows) for name in DETEXTURE_BINARY_SCALAR_FIELDS}
    summary = {
        "dataset_id": DETEXTURE_DATASET_ID,
...
def build_detexture_binary_markdown_summary(summary: dict[str, Any]) -> str:
    """Render a concise Markdown summary for DeTexture ADE20K."""
    ...
    for metric_name in DETEXTURE_BINARY_SCALAR_FIELDS:
        lines.append(
...
def save_detexture_binary_panel(
    output_path: str | Path,
    sample: DeTextureBinarySample,
    evaluation: DeTextureBinarySampleResult,
    variant: str,
) -> Path:
    """Save the registered preview panel used for DeTexture ADE20K."""
    ...
    if evaluation.boundary_refine_sweep is not None:
        return save_boundary_refine_sweep_panel(
            ...
            protocol=_detexture_protocol_label(variant),
            metric_summary=evaluation.metric_summary,
        )
...
def render_detexture_binary_panel(
    sample: DeTextureBinarySample,
    evaluation: DeTextureBinarySampleResult,
    variant: str,
):
...
def build_detexture_binary_visual_record(
    sample: DeTextureBinarySample,
    evaluation: DeTextureBinarySampleResult,
    variant: str,
    visual_path: Path,
) -> dict[str, Any]:
    """Build one machine-readable visual record for DeTexture ADE20K."""
...
def resolve_detexture_binary_output_dir(output_dir: str | None, variant: str) -> Path:
    """Resolve the output directory for a DeTexture ADE20K run."""

    if output_dir:
        requested_dir = Path(output_dir)
        if _normalize_output_path(requested_dir) == _normalize_output_path(DETEXTURE_BINARY_OUTPUT_ROOT):
            safe_dir = requested_dir / variant
            LOGGER.warning(
                "Requested shared DeTexture output root %s; redirecting this run to %s to avoid mixed artifacts.",
                requested_dir,
                safe_dir,
            )
            return safe_dir
    ...
    return DETEXTURE_BINARY_OUTPUT_ROOT / f"detexture-{variant}-{timestamp}"


def prepare_detexture_binary_output_dir(
    output_dir: str | None,
    *,
    variant: str,
    run_kind: str,
    ...
    """Resolve and validate a DeTexture ADE20K output directory."""

    if run_kind not in {"predict", "eval"}:
        raise ValueError(f"Unsupported run_kind '{run_kind}'. Expected 'predict' or 'eval'.")
    resolved_dir = resolve_detexture_binary_output_dir(output_dir, variant=variant)
    if output_dir and _normalize_output_path(Path(output_dir)) == _normalize_output_path(DETEXTURE_BINARY_OUTPUT_ROOT):
        if run_kind == "predict":
            resolved_dir = resolved_dir.with_name(f"{variant}_index{sample_index}")
        _validate_detexture_binary_output_dir(resolved_dir, requested_variant=variant, run_kind=run_kind)
        return resolved_dir
    _validate_detexture_binary_output_dir(resolved_dir, requested_variant=variant, run_kind=run_kind)
    return resolved_dir


def _detexture_protocol_label(variant: str) -> str:
    return f"detexture_binary:{variant}"
...
def _validate_detexture_binary_output_dir(
    output_dir: Path,
    *,
    ...
    for metadata_path in (output_dir / "config.json", output_dir / "summary.json", output_dir / "prediction.json"):
        ...
        if isinstance(existing_variant, str) and existing_variant != requested_variant:
            raise RuntimeError(
                f"Refusing to reuse DeTexture output directory '{output_dir}' because it belongs to variant "
