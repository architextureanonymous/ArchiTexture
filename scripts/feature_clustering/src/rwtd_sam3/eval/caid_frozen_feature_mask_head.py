"""Dense-supervised CAID baseline on frozen SAM multiscale features.

This module implements a minimal supervised baseline for CAID:
...
CAID_DATASET_ID = "architexture:caid"
CAID_ROUTE = "caid"
DATASET_DISPLAY_NAME = "CAID"
FOREGROUND_LABEL = "texture_a"
BACKGROUND_LABEL = "texture_b"
GLAS_DATASET_ID = CAID_DATASET_ID
GLAS_FROZEN_MASK_HEAD_OUTPUT_ROOT = Path("outputs") / "architexture_binary" / "frozen_sam_mask_head" / "caid"
...
def resolve_train_subset_manifest(args) -> FewShotSubsetManifest | None:
    ...
    return load_few_shot_subset_manifest(
        manifest_path,
        expected_dataset_id=CAID_DATASET_ID,
...
def run_glas_frozen_mask_head_eval(args) -> dict[str, Any]:
    ...
    if num_samples < 1:
        raise FrozenMaskHeadRuntimeError("The requested CAID eval split produced zero samples.")
            ...
            if args.save_visuals:
                visual_records.append(
                    build_visual_record(
                        ...
                        protocol=f"caid_frozen_mask_head:{variant}",
                        visual_path=Path("visuals") / f"{cached_sample.sample.index}.png",
                    )
                )

    if not rows:
        raise FrozenMaskHeadRuntimeError("Frozen-mask-head evaluation produced no CAID sample rows.")
...
def build_glas_frozen_mask_head_summary_markdown(summary: dict[str, Any]) -> str:
    lines = [
        "# CAID Frozen SAM Mask Head Summary",
        "",
...
def build_glas_frozen_mask_head_experiment_terms_markdown(
    ...
    *,
    ...
    sections = [
        ExperimentTermsSection(
            ...
            bullets=(
                ...
                f"Dataset: `{GLAS_DATASET_ID}` with route `{CAID_ROUTE}` from benchmark root `{getattr(args, 'benchmark_root', None)}`.",
        ...
        ExperimentTermsSection(
            ...
            bullets=(
                ...
                "The repo-native CAID loader requires the prepared split-aware root with `ImageSets/Segmentation/{train,test}.txt` and uses the repository's random 50/50 seed-0 split artifact.",
                (
                ...
                "Foreground polarity contract: the positive region corresponds to `texture_a_mask` in the CAID adapter.",
    ...
    return render_experiment_terms_markdown(
        title="CAID Frozen SAM Mask Head Experiment Terms",
        summary_lines=(
            "This file describes the exact frozen-feature supervised CAID run that produced this directory.",
        ...
        related_paths=(
            ...
            render_relative_path(Path("src/rwtd_sam3/eval/caid_frozen_feature_mask_head.py")),
        ),
    )
...
run_caid_frozen_mask_head_train = run_glas_frozen_mask_head_train
run_caid_frozen_mask_head_eval = run_glas_frozen_mask_head_eval
