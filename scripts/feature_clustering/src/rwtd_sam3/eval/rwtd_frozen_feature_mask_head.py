"""Dense-supervised RWTD baseline on frozen SAM multiscale features.

This module implements a minimal supervised baseline for RWTD:
...
from rwtd_sam3.data.rwtd import (
    DEFAULT_DATASET_ID,
    RwtdSample as GlasBinarySample,
    iter_rwtd_samples as iter_architexture_binary_samples,
    load_split_overview as load_architexture_binary_overview,
)
...
RWTD_DATASET_ID = "rwtd"
RWTD_ROUTE = "rwtd"
DATASET_DISPLAY_NAME = "RWTD"
FOREGROUND_LABEL = "texture_a"
BACKGROUND_LABEL = "texture_b"
GLAS_DATASET_ID = RWTD_DATASET_ID
GLAS_FROZEN_MASK_HEAD_OUTPUT_ROOT = Path("outputs") / "rwtd" / "frozen_sam_mask_head"
...
def load_glas_binary_overview(*, split: str, dataset_name: str, cache_dir: str | None):
    ...
    return type(
        ...
        {
            "route": RWTD_ROUTE,
            "benchmark_root": Path(dataset_name),
            "image_dir": None,
            "label_dir": None,
            "num_examples": int(overview.num_examples),
            "image_ids": tuple(),
            "evaluation_view": "partition_invariant",
            "split": split,
            "split_layout": "rwtd_dataset_split",
        },
    )()
...
def resolve_train_subset_manifest(args) -> FewShotSubsetManifest | None:
    ...
    return load_few_shot_subset_manifest(
        manifest_path,
        expected_dataset_id=RWTD_DATASET_ID,
...
def resolve_train_selection_policy(args) -> str:
    if resolve_train_subset_manifest(args) is None:
        return "rwtd_default_dataset_split"
    return "few_shot_subset_manifest_over_rwtd_default_dataset_split"
...
def resize_glas_binary_sample(sample: GlasBinarySample, *, resize_hw: tuple[int, int] | None) -> GlasBinarySample:
    """Resize one decoded RWTD sample for the mask-head path, if requested."""
    ...
    if not resized_foreground_mask.any() or not resized_background_mask.any():
        raise FrozenMaskHeadRuntimeError(
            "Resizing the RWTD sample produced an empty foreground or background mask.",
            diagnostics={
...
def compute_partition_invariant_bce_dice_loss(
    logits: Any,
    *,
    target_a_mask: Any,
    target_b_mask: Any,
...
) -> tuple[MaskHeadLossResult, str]:
    """Score both binary assignments and keep the lower-loss alignment."""

    direct = compute_bce_dice_loss(
        logits,
        target_a_mask,
    ...
    if float(swapped.loss.detach().cpu().item()) < float(direct.loss.detach().cpu().item()):
        return swapped, "foreground_head->texture_b,background_complement->texture_a"
    return direct, "foreground_head->texture_a,background_complement->texture_b"
...
def run_glas_frozen_mask_head_train(args) -> dict[str, Any]:
    """Train the tiny supervised mask head on frozen SAM features and evaluate on RWTD."""

    import gc

    if getattr(args, "train_limit", None) is not None and getattr(args, "train_subset_manifest", None) not in (None, ""):
        raise FrozenMaskHeadRuntimeError(
            "train_limit and train_subset_manifest are mutually exclusive for RWTD frozen-mask-head training.",
            diagnostics={
    ...
    summary["best_epoch_selection_metric"] = ("eval_miou" if training_artifacts.best_epoch_by_eval_dice is not None else None)
...
def run_glas_frozen_mask_head_eval(args) -> dict[str, Any]:
    """Evaluate one saved frozen-mask-head checkpoint on an RWTD split."""
    ...
    if num_samples < 1:
        raise FrozenMaskHeadRuntimeError("The requested RWTD eval split produced zero samples.")
    ...
    for epoch in range(int(args.num_epochs)):
        ...
        for sample_index in iteration_order:
            ...
            if is_coarse_plus_residual_variant(args.variant):
                ...
                final_loss_result, _ = compute_partition_invariant_bce_dice_loss(
                    residual_combination.final_logits,
                    target_a_mask=np.asarray(cached_sample.supervision_foreground_mask, dtype=np.float32),
                    target_b_mask=np.asarray(cached_sample.supervision_background_mask, dtype=np.float32),
                ...
                coarse_loss_result, _ = compute_partition_invariant_bce_dice_loss(
                    residual_combination.coarse_logits,
                    target_a_mask=np.asarray(cached_sample.supervision_foreground_mask, dtype=np.float32),
                    target_b_mask=np.asarray(cached_sample.supervision_background_mask, dtype=np.float32),
                    bce_weight=float(args.bce_weight),
                    dice_weight=float(args.dice_weight),
                ...
                residual_l1_loss = residual_combination.residual_logits.abs().mean()
                total_loss = final_loss_result.loss
                if float(getattr(args, "coarse_loss_weight", 0.0)) != 0.0:
                    total_loss = total_loss + float(getattr(args, "coarse_loss_weight", 0.0)) * coarse_loss_result.loss
                if float(getattr(args, "residual_l1_weight", 0.0)) != 0.0:
                    total_loss = total_loss + float(getattr(args, "residual_l1_weight", 0.0)) * residual_l1_loss
                if (
                    float(getattr(args, "attention_sparsity_weight", 0.0)) != 0.0
                    and residual_combination.attention_map is not None
                ):
                    total_loss = total_loss + float(getattr(args, "attention_sparsity_weight", 0.0)) * residual_combination.attention_map.mean()
                loss_result = final_loss_result
            else:
                loss_result, _assignment_used = compute_partition_invariant_bce_dice_loss(
                    output.logits,
                    target_a_mask=np.asarray(cached_sample.supervision_foreground_mask, dtype=np.float32),
                    target_b_mask=np.asarray(cached_sample.supervision_background_mask, dtype=np.float32),
                    bce_weight=float(args.bce_weight),
        ...
        if should_run_epoch_eval:
            ...
            if best_eval_dice is None or float(epoch_eval_row["eval_miou"]) > float(best_eval_dice):
                best_eval_dice = float(epoch_eval_row["eval_miou"])
            ...
            LOGGER.info(
                "Frozen-mask-head epoch-eval=%d | eval_miou=%.6f eval_ari=%.6f",
                epoch + 1,
...
def evaluate_glas_frozen_mask_head_on_cache(
    *,
    ...
    """Evaluate one trained frozen mask head on one cached RWTD split."""
    ...
    for cached_sample in iter_cached_glas_feature_samples(cache):
        ...
        rows.append(row)
        metric_summary = (
            f"mIoU={row['eval_miou']:.3f} "
            f"ARI={row['eval_ari']:.3f} "
            f"TexA IoU={row['texture_a_best_iou']:.3f} "
            f"TexB IoU={row['texture_b_best_iou']:.3f}"
        )
            ...
            if args.save_visuals:
                visual_records.append(
                    build_visual_record(
                        ...
                        protocol=f"rwtd_frozen_mask_head:{variant}",
                        visual_path=Path("visuals") / f"{cached_sample.sample.index}.png",
                    )
                )

    if not rows:
        raise FrozenMaskHeadRuntimeError("Frozen-mask-head evaluation produced no RWTD sample rows.")
...
def build_glas_frozen_mask_head_eval_row(
    *,
    ...
    row: dict[str, Any] = {
        ...
        "checkpoint_path": str(checkpoint_path),
        "foreground_evaluation_view": "partition_invariant",
        "foreground_assignment_used": assignment.assignment_used,
        ...
        "predicted_positive_fraction": float(np.asarray(assignment.chosen_prediction_a, dtype=bool).sum() / total_pixels),
        "target_positive_fraction": float(np.asarray(cached_sample.sample.texture_a_mask, dtype=bool).sum() / total_pixels),
        ...
        "texture_a_best_iou": float(
            compute_binary_metrics(
                np.asarray(assignment.chosen_prediction_a, dtype=bool),
                np.asarray(cached_sample.sample.texture_a_mask, dtype=bool),
            ).iou
        ),
        "texture_b_best_iou": float(
            compute_binary_metrics(
                np.asarray(assignment.chosen_prediction_b, dtype=bool),
                np.asarray(cached_sample.sample.texture_b_mask, dtype=bool),
            ).iou
        ),
    }
...
def build_glas_frozen_mask_head_summary(
    *,
    ...
    return {
        ...
        "aux_partition_primary_metric_name": "texture_a_best_iou",
        "aux_partition_secondary_metric_name": "texture_b_best_iou",
...
def flatten_glas_frozen_mask_head_summary(summary: dict[str, Any]) -> dict[str, Any]:
    flattened = {
        ...
        "texture_a_best_iou": summary["mean_metrics"]["texture_a_best_iou"],
        "texture_b_best_iou": summary["mean_metrics"]["texture_b_best_iou"],
...
def build_glas_frozen_mask_head_summary_markdown(summary: dict[str, Any]) -> str:
    lines = [
        "# RWTD Frozen SAM Mask Head Summary",
        "",
        ...
        f"- Mean partition-invariant mIoU: `{summary['mean_metrics']['eval_miou']:.6f}`",
        f"- Mean partition-invariant ARI: `{summary['mean_metrics']['eval_ari']:.6f}`",
        f"- Mean texture-a aligned IoU: `{summary['mean_metrics']['texture_a_best_iou']:.6f}`",
        f"- Mean texture-b aligned IoU: `{summary['mean_metrics']['texture_b_best_iou']:.6f}`",
    ]
    if summary.get("train_set_mean_metrics") is not None:
        lines.append(f"- Mean train-set partition mIoU: `{summary['train_set_mean_metrics']['eval_miou']:.6f}`")
        lines.append(f"- Mean train-set partition ARI: `{summary['train_set_mean_metrics']['eval_ari']:.6f}`")
...
def build_glas_frozen_mask_head_run_config(
    ...
    *,
    ...
    config = {
        ...
        "run_kind": run_kind,
        "dataset_id": getattr(args, "dataset_id", GLAS_DATASET_ID),
        "route": RWTD_ROUTE,
    ...
    sections = [
        ExperimentTermsSection(
            ...
            bullets=(
                ...
                f"Dataset: `{GLAS_DATASET_ID}` using dataset id `{getattr(args, 'dataset_id', None)}`.",
...
run_rwtd_frozen_mask_head_train = run_glas_frozen_mask_head_train
run_rwtd_frozen_mask_head_eval = run_glas_frozen_mask_head_eval
