"""Dense-supervised MoNuSeg baseline on frozen SAM multiscale features.

This module implements a minimal supervised baseline for MoNuSeg:

- the SAM backbone stays frozen
- one tiny multiscale mask head is trained on dense nucleus masks
...
from rwtd_sam3.data.monuseg_binary import (
    MONUSEG_DATASET_ID as GLAS_DATASET_ID,
    MONUSEG_HF_DATASET_NAME,
    MonusegBinarySample as GlasBinarySample,
    iter_monuseg_binary_samples as iter_glas_binary_samples,
    load_monuseg_binary_overview as load_glas_binary_overview,
)
...
from rwtd_sam3.eval.evaluation_contract import DIRECT_FOREGROUND_PRIMARY, UPSTREAM_FAITHFUL_PRIMARY
...
from rwtd_sam3.eval.monuseg_autosam import (
    ...
    compute_upstream_binary_iou_dice,
)
...
DATASET_DISPLAY_NAME = "MoNuSeg"
FOREGROUND_LABEL = "nucleus"
BACKGROUND_LABEL = "background"
GLAS_FROZEN_MASK_HEAD_OUTPUT_ROOT = Path("outputs") / "monuseg_binary" / "frozen_sam_mask_head"
...
GLAS_FROZEN_MASK_HEAD_SELECTION_CHECKPOINT_MODES = ("final", "best_eval")
...
MONUSEG_FROZEN_MASK_HEAD_CONTRACT_PROFILE = {
    "profile_name": "sam3_historical_512_full_train_v1",
    "train_split_policy": "official_full_train_unless_validation_holdout_requested",
    "resize_height": 512,
    "resize_width": 512,
...
def build_frozen_mask_head_wandb_preview(
    *,
    ...
    return render_prediction_panel(
        ...
        protocol=f"monuseg_frozen_mask_head:{variant}",
        metric_summary=metric_summary,
    )
...
class CachedGlasFeatureSample:
    """One decoded MoNuSeg sample plus cached raw frozen SAM pyramid features."""
...
class GlasFeatureCache:
    """Cached frozen SAM features for one MoNuSeg split and one scale selection."""
...
def resolve_mask_head_selection_split(args) -> str:
    explicit = getattr(args, "selection_split", None)
    if explicit not in (None, ""):
        return str(explicit)
    if resolve_mask_head_validation_holdout_count(args) > 0:
        return "val"
    return "train"


def resolve_mask_head_selection_metric(args) -> str:
    metric_name = str(getattr(args, "selection_metric", "direct_foreground_dice"))
    if metric_name not in GLAS_FROZEN_MASK_HEAD_SELECTION_METRICS:
        raise FrozenMaskHeadRuntimeError(
            f"Unsupported frozen-mask-head selection metric '{metric_name}'.",
            diagnostics={"supported_selection_metrics": list(GLAS_FROZEN_MASK_HEAD_SELECTION_METRICS)},
        )
    return metric_name


def resolve_mask_head_selection_checkpoint_mode(args) -> str:
    checkpoint_mode = str(getattr(args, "selection_checkpoint_mode", "final"))
    if checkpoint_mode not in GLAS_FROZEN_MASK_HEAD_SELECTION_CHECKPOINT_MODES:
        raise FrozenMaskHeadRuntimeError(
            f"Unsupported frozen-mask-head selection_checkpoint_mode '{checkpoint_mode}'.",
            diagnostics={"supported_selection_checkpoint_modes": list(GLAS_FROZEN_MASK_HEAD_SELECTION_CHECKPOINT_MODES)},
        )
    return checkpoint_mode


def resolve_mask_head_resolved_split_policy(args) -> str:
    holdout_count = int(getattr(args, "validation_holdout_count", 0))
    if holdout_count < 0:
        raise FrozenMaskHeadRuntimeError(
            "validation_holdout_count must be non-negative.",
            diagnostics={"validation_holdout_count": holdout_count},
        )
    if holdout_count == 0:
        return "official_full_train"
    return "official_train_minus_validation_holdout"


def resolve_mask_head_validation_holdout_count(args) -> int:
    return int(getattr(args, "validation_holdout_count", 0))


def resolve_mask_head_validation_subset_seed(args) -> int:
    return int(getattr(args, "validation_subset_seed", MONUSEG_CONTRACT_VAL_SUBSET_SEED))


def validate_best_eval_requires_cadence(args) -> None:
    if resolve_mask_head_selection_checkpoint_mode(args) == "best_eval" and resolve_mask_head_eval_every_epochs(args) is None:
        raise FrozenMaskHeadRuntimeError(
...
def validate_sam3_checkpoint_contract(args) -> None:
    ...
    if "sam_vit_h" in path_text or "vit_h" in path_text:
        raise FrozenMaskHeadRuntimeError(
            "SAM3 frozen-feature routes do not accept the AutoSAM ViT-H checkpoint path. "
            "Use a SAM3-native checkpoint/config path for this route and keep ViT-H only on the AutoSAM faithful route.",
...
def resolve_mask_head_upstream_eval_image_size(args) -> int:
    value = int(
        getattr(
            args,
            "upstream_eval_image_size",
            MONUSEG_AUTOSAM_REPRODUCTION_PROFILE["prompt_image_size"],
        )
    )
    if value < 1:
        raise FrozenMaskHeadRuntimeError(
            "upstream_eval_image_size must be positive.",
            diagnostics={"upstream_eval_image_size": value},
        )
    return value
...
def resolve_train_selection_policy(args) -> str:
    ...
    return "few_shot_subset_manifest_over_official_challenge_train_excludes_tissue_0_unknown"


def resize_glas_binary_sample(sample: GlasBinarySample, *, resize_hw: tuple[int, int] | None) -> GlasBinarySample:
    """Resize one decoded MoNuSeg sample for the mask-head path, if requested."""
    ...
    if not resized_foreground_mask.any() or not resized_background_mask.any():
        raise FrozenMaskHeadRuntimeError(
            "Resizing the MoNuSeg sample produced an empty nucleus or background mask.",
            diagnostics={
...
def apply_backbone_preprocessing_to_glas_binary_sample(
    ...
    *,
    ...
    target_height, target_width = _resize_longest_side_hw(int(sample.height), int(sample.width), AUTOSAM_STYLE_BACKBONE_LONG_SIDE)
    resized_image = sample.image.convert("RGB").resize((int(target_width), int(target_height)), resample=Image.Resampling.BILINEAR)
    ...
    if not resized_foreground_mask.any() or not resized_background_mask.any():
        raise FrozenMaskHeadRuntimeError(
            "AutoSAM-style backbone preprocessing produced an empty nucleus or background mask.",
            diagnostics={
                "crop_name": sample.crop_name,
...
def _resize_longest_side_hw(height: int, width: int, long_side: int) -> tuple[int, int]:
    if height < 1 or width < 1 or long_side < 1:
        raise FrozenMaskHeadRuntimeError(
            "Resize-longest-side dimensions must be positive.",
            diagnostics={"height": height, "width": width, "long_side": long_side},
        )
...
def compute_upstream_eval_metrics_for_probability_map(
    *,
    foreground_probability: np.ndarray,
    ...
    """Apply the AutoSAM-style resized-binary eval view to a foreground probability map."""

    target_size = (int(image_size), int(image_size))
    prediction_image = Image.fromarray(np.asarray(foreground_probability, dtype=np.float32), mode="F")
    resized_prediction = np.asarray(
        prediction_image.resize(target_size, resample=Image.Resampling.BILINEAR),
        dtype=np.float32,
    )
    target_image = Image.fromarray((np.asarray(target_foreground, dtype=np.uint8) * 255), mode="L")
    resized_target = (
        np.asarray(
            target_image.resize(target_size, resample=Image.Resampling.NEAREST),
            dtype=np.uint8,
        )
        > 0
    )
    resized_prediction_binary = np.asarray(resized_prediction >= float(foreground_threshold), dtype=bool)
    upstream_iou, upstream_dice = compute_upstream_binary_iou_dice(
        resized_prediction_binary,
        resized_target,
    )
    return resized_prediction_binary, resized_target, float(upstream_iou), float(upstream_dice)
...
def run_glas_frozen_mask_head_train(args) -> dict[str, Any]:
    """Train the tiny supervised mask head on frozen SAM features and evaluate on MoNuSeg."""
    ...
    if getattr(args, "train_limit", None) is not None and getattr(args, "train_subset_manifest", None) not in (None, ""):
        raise FrozenMaskHeadRuntimeError(
            "train_limit and train_subset_manifest are mutually exclusive for MoNuSeg frozen-mask-head training.",
            diagnostics={
    ...
    selection_metric = resolve_mask_head_selection_metric(args)
    selection_checkpoint_mode = resolve_mask_head_selection_checkpoint_mode(args)
    selection_split = resolve_mask_head_selection_split(args)
    validation_holdout_count = resolve_mask_head_validation_holdout_count(args)
    validation_subset_seed = resolve_mask_head_validation_subset_seed(args)
    ...
    extractor = build_frozen_sam_pyramid_extractor(
        model_id=getattr(args, "model_id", "sam-vit-h"),
        device=args.device,
    ...
    selection_cache: GlasFeatureCache | None = None
    if eval_every_epochs is not None:
        selection_cache = build_glas_feature_cache(
            extractor=extractor,
            dataset_name=args.dataset_name,
            cache_dir=getattr(args, "cache_dir", None),
    ...
    eval_cache: GlasFeatureCache
    if selection_cache is not None and selection_split == str(args.eval_split):
        eval_cache = selection_cache
    else:
    ...
    config = build_glas_frozen_mask_head_run_config(
        ...
        materialization_mode=(
            f"train={train_materialization_mode};"
            f"selection={(selection_cache.materialization_mode if selection_cache is not None else 'none')};"
            f"eval={eval_cache.materialization_mode}"
        ),
    )
    ...
    wandb_session = WandbSession(
        ...
        project=str(getattr(args, "wandb_project", "monuseg-frozen-sam3")),
    ...
    try:
        head_module, training_artifacts = train_glas_frozen_mask_head(
            ...
            selection_cache_for_tracking=selection_cache,
        ...
        train_summary = evaluate_glas_frozen_mask_head_on_cache(
            ...
            allow_train_selection=validation_holdout_count < 1,
        )
        write_json(output_dir / "train_set_summary.json", train_summary)
        del train_cache
        gc.collect()
        final_checkpoint = torch.load(training_artifacts.checkpoint_path, map_location="cpu")
    except Exception:
        wandb_session.finish()
        raise
    final_head_module = build_frozen_mask_head_from_checkpoint(final_checkpoint)
    try:
        final_head_module.load_state_dict(final_checkpoint["head_state_dict"])
    except RuntimeError as exc:
        raise FrozenMaskHeadRuntimeError(
            "Loaded MoNuSeg frozen-mask-head final checkpoint state_dict is incompatible with the instantiated architecture.",
            diagnostics={"checkpoint_path": str(training_artifacts.checkpoint_path), "variant": args.variant},
        ) from exc
    final_summary = evaluate_glas_frozen_mask_head_on_cache(
        head_module=final_head_module,
        cache=eval_cache,
        variant=args.variant,
        args=args,
        checkpoint_path=training_artifacts.checkpoint_path,
    ...
    write_text(output_dir / "final_checkpoint_summary.md", build_glas_frozen_mask_head_summary_markdown(final_summary))

    headline_checkpoint_path = training_artifacts.checkpoint_path
    headline_checkpoint_role = "final"
    if (
        selection_checkpoint_mode == "best_eval"
        and training_artifacts.best_checkpoint_path is not None
    ):
        headline_checkpoint_path = training_artifacts.best_checkpoint_path
        headline_checkpoint_role = "best_eval"
    headline_checkpoint = torch.load(headline_checkpoint_path, map_location="cpu")
    headline_head_module = build_frozen_mask_head_from_checkpoint(headline_checkpoint)
    try:
        headline_head_module.load_state_dict(headline_checkpoint["head_state_dict"])
    except RuntimeError as exc:
        raise FrozenMaskHeadRuntimeError(
            "Loaded MoNuSeg frozen-mask-head headline checkpoint state_dict is incompatible with the instantiated architecture.",
            diagnostics={"checkpoint_path": str(headline_checkpoint_path), "variant": args.variant},
        ) from exc
    summary = evaluate_glas_frozen_mask_head_on_cache(
        head_module=headline_head_module,
        cache=eval_cache,
        variant=args.variant,
        args=args,
    ...
    wandb_session.log(
        {
            "final/upstream_iou": summary["mean_metrics"]["upstream_eval_iou"],
            "final/upstream_dice": summary["mean_metrics"]["upstream_eval_dice"],
    ...
    summary["selection_split"] = selection_split
    summary["selection_checkpoint_mode"] = selection_checkpoint_mode
    ...
    summary["best_epoch_selection_metric"] = (
        selection_metric if training_artifacts.best_epoch_by_selection_metric is not None else None
    )
    summary["best_epoch_selection_caveat"] = (
        "analysis_only_when_eval_split_is_test"
        if training_artifacts.best_epoch_by_selection_metric is not None and str(selection_split) == "test"
        else None
    )
    summary["final_checkpoint_path"] = str(training_artifacts.checkpoint_path)
    ...
    rewrite_contract_summary(
        output_dir=output_dir,
        summary=summary,
        summary_markdown_builder=build_glas_frozen_mask_head_summary_markdown,
        summary_csv_flattener=flatten_glas_frozen_mask_head_summary,
    )
    return summary


def run_glas_frozen_mask_head_eval(args) -> dict[str, Any]:
    """Evaluate one saved frozen-mask-head checkpoint on a MoNuSeg split."""
    ...
    setattr(args, "selection_metric", str(checkpoint.get("selection_metric", "direct_foreground_dice")))
    setattr(args, "selection_split", str(checkpoint.get("selection_split", "val")))
    setattr(args, "selection_checkpoint_mode", str(checkpoint.get("selection_checkpoint_mode", "final")))
    setattr(
        args,
        "upstream_eval_image_size",
        int(checkpoint.get("upstream_eval_image_size", MONUSEG_AUTOSAM_REPRODUCTION_PROFILE["prompt_image_size"])),
    )
    setattr(
        args,
        "boundary_weight",
    ...
    if num_samples < 1:
        raise FrozenMaskHeadRuntimeError("The requested MoNuSeg eval split produced zero samples.")
    ...
    try:
        ...
        raise FrozenMaskHeadRuntimeError(
            "Loaded MoNuSeg frozen-mask-head checkpoint state_dict is incompatible with the instantiated architecture.",
...
def load_monuseg_contract_samples(
    *,
    split: str,
    dataset_name: str,
    cache_dir: str | None,
...
def resolve_monuseg_validation_manifest_dict(
    *,
    ...
    if int(validation_holdout_count) < 1:
        return {
            "dataset_id": "monuseg",
            ...
            "source_id_order": [str(sample.crop_name) for sample in official_train_samples],
            "sampled_prefix_ids": [],
        }
...
def build_glas_feature_cache(
    *,
    ...
    """Decode MoNuSeg samples and cache the raw frozen SAM pyramid levels."""
    ...
    raw_samples = tuple(
        ...
        for sample in load_monuseg_contract_samples(
            split=split,
            dataset_name=dataset_name,
            cache_dir=cache_dir,
            limit=limit,
            ...
            validation_holdout_count=int(validation_holdout_count),
            validation_subset_seed=int(validation_subset_seed),
        )
    )
    ...
    if selected_level_names is None or reference_raw_level_shapes is None or reference_selected_level_shapes is None:
        raise FrozenMaskHeadRuntimeError("Failed to build a non-empty MoNuSeg frozen-feature cache.")

    LOGGER.info(
        "Prepared %d MoNuSeg samples for split=%s | levels=%s | materialization_mode=%s | estimated_bundle_mb=%.2f | estimated_total_gb=%.2f | reference_raw_shapes=%s | reference_selected_shapes=%s | resize_hw=%s | backbone_preprocessing_mode=%s",
        len(raw_samples),
        split,
...
def build_frozen_mask_head_from_cache(cache: GlasFeatureCache, *, variant: str, args=None) -> Any:
    ...
    if is_serialized_local_routed_refine_variant(variant):
        ...
        return FrozenSamSerializedLocalRoutedRefinementMaskHead(
            ...
            residual_warmup_epochs=int(getattr(args, "joker_residual_warmup_epochs", 40)),
            residual_ramp_epochs=int(getattr(args, "joker_residual_ramp_epochs", 20)),
...
def prepare_cached_glas_feature_sample(
    *,
    ...
    """Extract and validate one MoNuSeg sample's raw frozen SAM multiscale features."""
    ...
    if tuple(int(value) for value in image_size) != expected_image_size:
        raise FrozenMaskHeadRuntimeError(
            "Frozen-mask-head feature extraction returned an image size that does not match the decoded MoNuSeg sample.",
            diagnostics={
    ...
    if reference_channels is not None and current_channels != reference_channels:
        raise FrozenMaskHeadRuntimeError(
            "Discovered inconsistent SAM channel counts across MoNuSeg samples.",
            diagnostics={
                "reference_channels": reference_channels,
...
def estimate_cached_glas_feature_sample_nbytes(sample: CachedGlasFeatureSample) -> int:
    """Estimate the raw CPU memory footprint of one cached MoNuSeg feature sample."""
...
def iter_cached_glas_feature_samples(cache: GlasFeatureCache):
    """Yield prepared MoNuSeg feature samples, streaming re-extraction when needed."""
...
def materialize_cached_feature_levels_on_device(
    ...
    *,
    ...
    """Move one cached MoNuSeg sample's selected raw feature pyramid onto a torch device."""
    ...
    for level_name in sample.selected_level_names:
        ...
        if feature_map.ndim != 3:
            raise FrozenMaskHeadRuntimeError(
                f"Cached MoNuSeg feature map must have shape [C,H,W], got {feature_map.shape} for {level_name}."
            )
        raw = torch.from_numpy(np.ascontiguousarray(feature_map[None]))
        if non_blocking and str(device) != "cpu":
            raw = raw.pin_memory()
        tensor = raw.to(device, non_blocking=non_blocking)
        if tensor.ndim != 4 or int(tensor.shape[0]) != 1:
            raise FrozenMaskHeadRuntimeError(
                f"Materialized MoNuSeg feature tensor must have shape [1,C,H,W], got {tuple(int(value) for value in tensor.shape)}."
            )
        materialized[level_name] = tensor
    return materialized


def _iter_prefetched_training_samples(
    ...
    *,
    ...
    """Yield cached feature samples with SAM backbone run in a background thread.

    The worker thread runs augmentation + SAM backbone extraction for the next
    sample while the main thread trains the head on the current one, overlapping
    GPU compute across samples.  Queue depth is capped at *prefetch_count* to
    bound memory usage.
    """
    ...
    _SENTINEL = object()
    q: _queue.Queue = _queue.Queue(maxsize=prefetch_count)
    ...
    while True:
        ...
        if isinstance(item, Exception):
            raise FrozenMaskHeadRuntimeError(
                f"Prefetch worker raised an exception: {item!r}"
            ) from item
        yield item
...
def train_glas_frozen_mask_head(
    *,
    ...
    """Optimize only the tiny supervised mask head on cached frozen MoNuSeg features."""
    ...
    best_epoch_by_selection_metric: int | None = None
    best_selection_metric_value: float | None = None
    best_epoch_by_eval_dice: int | None = None
    best_eval_dice: float | None = None
    best_state_dict: dict[str, Any] | None = None

    LOGGER.info(
        "Frozen-mask-head training start | variant=%s levels=%s materialization_mode=%s trainable_params=%d lr=%.6f wd=%.6f epochs=%d train_aug_policy=%s loss_variant=%s boundary_weight=%.3f eval_every_epochs=%s selection_metric=%s residual_gate_mode=%s residual_gate_threshold=%.3f coarse_loss_weight=%.3f residual_l1_weight=%.3f attention_sparsity_weight=%.3f joker_zero_res=%s joker_zero_qkv=%s joker_warmup=%d joker_ramp=%d",
        args.variant,
    ...
    for epoch in range(int(args.num_epochs)):
        ...
        prefetch_count = int(getattr(args, "num_workers", 1))
        # Use threading prefetcher to overlap SAM extraction with head optimization
        sample_iterator = _iter_prefetched_training_samples(
        ...
        for cached_sample in sample_iterator:
            feature_levels = materialize_cached_feature_levels_on_device(cached_sample, device=device)
        ...
        if hasattr(head_module, "alpha_fpn1"):
            history_row["alpha_fpn1"] = float(head_module.alpha_fpn1)
            history_row["alpha_fpn0"] = float(head_module.alpha_fpn0)
        if hasattr(head_module, "scale_fpn1"):
            history_row["scale_fpn1"] = float(head_module.scale_fpn1.detach().cpu().item())
            history_row["scale_fpn0"] = float(head_module.scale_fpn0.detach().cpu().item())
            ...
            wandb_session.log(
                {
                    ...
                    "model/train_alpha_fpn1": history_row.get("alpha_fpn1"),
                    "model/train_alpha_fpn0": history_row.get("alpha_fpn0"),
                    "model/train_scale_fpn1": history_row.get("scale_fpn1"),
                    "model/train_scale_fpn0": history_row.get("scale_fpn0"),
                },
                step=epoch + 1,
            )
        LOGGER.info(
        ...
        if should_run_epoch_eval:
            ...
            if selection_cache_for_tracking is not None:
                selection_summary = evaluate_glas_frozen_mask_head_on_cache(
                    head_module=head_module,
                    cache=selection_cache_for_tracking,
                    variant=args.variant,
                    args=args,
                    checkpoint_path=epoch_checkpoint_path,
                    run_kind="epoch_tracking_selection_eval",
                    trainable_parameter_count=trainable_parameter_count,
                    output_dir=None,
                    save_artifacts=False,
                    allow_train_selection=int(validation_holdout_count) < 1,
                )
            else:
                selection_summary = epoch_summary
            epoch_eval_row = {
                "epoch": epoch + 1,
                "upstream_eval_iou": float(epoch_summary["mean_metrics"]["upstream_eval_iou"]),
                "upstream_eval_dice": float(epoch_summary["mean_metrics"]["upstream_eval_dice"]),
            ...
            residual_diagnostics_mean = epoch_summary.get("residual_diagnostics_mean")
                ...
                epoch_eval_row.update(
                    {
                        ...
                        "residual_scale_fpn1": residual_diagnostics_mean.get("residual_scale_fpn1"),
                        "residual_scale_fpn0": residual_diagnostics_mean.get("residual_scale_fpn0"),
                        "alpha_fpn1": residual_diagnostics_mean.get("alpha_fpn1"),
                        "alpha_fpn0": residual_diagnostics_mean.get("alpha_fpn0"),
                    }
                )
            ...
            if wandb_session is not None:
                wandb_session.log(
                    {
                        "eval/upstream_iou": epoch_eval_row["upstream_eval_iou"],
                        "eval/upstream_dice": epoch_eval_row["upstream_eval_dice"],
                ...
                if preview_sample is not None and bool(getattr(wandb_session, "enabled", False)):
                    preview_metric_summary = (
                        f"epoch={epoch + 1} upstream_iou={epoch_eval_row['upstream_eval_iou']:.3f} "
                        f"fg_iou={epoch_eval_row['direct_foreground_iou']:.3f} "
            ...
            selection_metric_value = float(selection_summary["mean_metrics"][selection_metric])
            if best_selection_metric_value is None or selection_metric_value > float(best_selection_metric_value):
                best_selection_metric_value = float(selection_metric_value)
                best_epoch_by_selection_metric = int(epoch + 1)
                best_state_dict = {
            ...
            LOGGER.info(
                "Frozen-mask-head epoch-eval=%d | upstream_iou=%.6f upstream_dice=%.6f eval_fg_iou=%.6f eval_dice=%.6f eval_miou=%.6f eval_ari=%.6f s1=%.4f s0=%.4f a1=%.2f a0=%.2f attn_f1=%s attn_f0=%s selection=%s value=%.6f",
                epoch + 1,
                epoch_eval_row["upstream_eval_iou"],
                epoch_eval_row["upstream_eval_dice"],
    ...
    checkpoint_payload = {
        ...
        "selection_split": resolve_mask_head_selection_split(args),
        "selection_checkpoint_mode": resolve_mask_head_selection_checkpoint_mode(args),
        "upstream_eval_image_size": resolve_mask_head_upstream_eval_image_size(args),
        "best_epoch_by_selection_metric": best_epoch_by_selection_metric,
    ...
    if best_state_dict is not None and best_epoch_by_selection_metric is not None:
        ...
        best_checkpoint_payload["checkpoint_kind"] = "best_eval_selection"
...
def evaluate_glas_frozen_mask_head_on_cache(
    *,
    ...
    """Evaluate one trained frozen mask head on one cached MoNuSeg split."""
    ...
    for cached_sample in iter_cached_glas_feature_samples(cache):
        ...
        _upstream_prediction_binary, _upstream_target_binary, upstream_eval_iou, upstream_eval_dice = (
            compute_upstream_eval_metrics_for_probability_map(
                foreground_probability=foreground_probability,
                target_foreground=np.asarray(cached_sample.sample.texture_a_mask, dtype=bool),
                image_size=resolve_mask_head_upstream_eval_image_size(args),
                foreground_threshold=float(args.foreground_threshold),
            )
        )
        ...
        if residual_combination is not None or is_serialized_local_routed_refine_variant(variant):
            ...
            residual_summary = (
                ...
                if residual_combination is not None
                    ...
                    "route1_coverage": (
                        float(output.route1_mask.float().mean().cpu().item())
                        if getattr(output, "route1_mask", None) is not None else None
                    ),
                    "route0_coverage": (
                        float(output.route0_mask.float().mean().cpu().item())
                        if getattr(output, "route0_mask", None) is not None else None
                    ),
        ...
        metric_summary = (
            ...
            f"Fg Dice={row['direct_foreground_dice']:.3f} "
    ...
    if not rows:
        raise FrozenMaskHeadRuntimeError("Frozen-mask-head evaluation produced no MoNuSeg sample rows.")
    ...
    if save_artifacts and output_dir is not None:
        ...
        summary = evaluate_run(
            ...
            split_role=str(cache.split),
            rows=rows,
            summary=summary,
            protocol_family="repo_comparable",
            primary_metric_pair=(
                UPSTREAM_FAITHFUL_PRIMARY
                if selection_metric.startswith("upstream_eval_")
                else DIRECT_FOREGROUND_PRIMARY
            ),
            selection_split=resolve_mask_head_selection_split(args),
            headline_split="test",
...
def profile_glas_frozen_mask_head_inference(
    *,
    ...
    if not timings_seconds:
        raise FrozenMaskHeadRuntimeError("Frozen-mask-head inference profiling received zero MoNuSeg samples.")
    return {
...
def build_glas_frozen_mask_head_eval_row(
    *,
    ...
    residual_combination: ResidualHeadCombinationResult | None = None,
    coarse_direct_foreground_metrics=None,
    coarse_upstream_eval_iou: float | None = None,
    coarse_upstream_eval_dice: float | None = None,
    coarse_assignment=None,
    ...
    row: dict[str, Any] = {
        ...
        "checkpoint_path": str(checkpoint_path),
        "foreground_evaluation_view": "direct_nucleus_foreground",
        "upstream_evaluation_view": "upstream_eval_resized_binary",
        "upstream_eval_image_size": int(upstream_eval_image_size),
        "foreground_assignment_used": "foreground_head->nucleus,background_complement->background",
    ...
    if residual_combination is not None or (
        coarse_direct_foreground_metrics is not None and coarse_assignment is not None
    ):
        ...
        if coarse_upstream_eval_iou is not None and coarse_upstream_eval_dice is not None:
            coarse_row["coarse_upstream_eval_iou"] = float(coarse_upstream_eval_iou)
            coarse_row["coarse_upstream_eval_dice"] = float(coarse_upstream_eval_dice)
            coarse_row["final_minus_coarse_upstream_eval_iou"] = float(upstream_eval_iou - coarse_upstream_eval_iou)
            coarse_row["final_minus_coarse_upstream_eval_dice"] = float(upstream_eval_dice - coarse_upstream_eval_dice)
    ...
    *,
    ...
    if residual_variant:
        coarse_mean_metrics = {
            ...
            "coarse_upstream_eval_iou": mean_optional("coarse_upstream_eval_iou"),
            "coarse_upstream_eval_dice": mean_optional("coarse_upstream_eval_dice"),
            "final_minus_coarse_upstream_eval_iou": mean_optional("final_minus_coarse_upstream_eval_iou"),
            "final_minus_coarse_upstream_eval_dice": mean_optional("final_minus_coarse_upstream_eval_dice"),
        }
        residual_diagnostics_mean = {
            ...
            "alpha_fpn1": mean_optional("alpha_fpn1"),
            "alpha_fpn0": mean_optional("alpha_fpn0"),
            "residual_scale_fpn1": mean_optional("residual_scale_fpn1"),
            "residual_scale_fpn0": mean_optional("residual_scale_fpn0"),
            "route1_coverage": mean_optional("route1_coverage"),
            "route0_coverage": mean_optional("route0_coverage"),
        }
    selection_metric = resolve_mask_head_selection_metric(args)
    if selection_metric.startswith("upstream_eval_"):
        ...
        headline_evaluation_view = "direct_nucleus_foreground"
        primary_metric_name = "direct_foreground_iou"
        secondary_metric_name = "direct_foreground_dice"
        repo_native_primary_metric_name = None
        repo_native_secondary_metric_name = None
    ...
    return {
        ...
        "validation_holdout_count": resolve_mask_head_validation_holdout_count(args),
        "validation_subset_seed": resolve_mask_head_validation_subset_seed(args),
        "dataset_name": str(cache.dataset_name),
        ...
        "validation_split_policy": describe_monuseg_validation_policy(
            holdout_count=resolve_mask_head_validation_holdout_count(args),
            subset_seed=resolve_mask_head_validation_subset_seed(args),
        ),
        "resolved_split_policy": resolve_mask_head_resolved_split_policy(args),
        "resolved_contract_profile": MONUSEG_FROZEN_MASK_HEAD_CONTRACT_PROFILE["profile_name"],
        "model_id": args.model_id,
        "device": args.device,
        "official_checkpoint_path": args.official_checkpoint_path,
        "checkpoint_path": str(checkpoint_path),
        ...
        "foreground_evaluation_view": "direct_nucleus_foreground",
        "headline_evaluation_view": headline_evaluation_view,
        "primary_metric_name": primary_metric_name,
        "secondary_metric_name": secondary_metric_name,
        "repo_native_primary_metric_name": repo_native_primary_metric_name,
        "repo_native_secondary_metric_name": repo_native_secondary_metric_name,
        "checkpoint_selection_materialized": (
            "best_eval"
            if resolve_mask_head_selection_checkpoint_mode(args) == "best_eval"
            and resolve_mask_head_eval_every_epochs(args) is not None
            and resolve_mask_head_validation_holdout_count(args) > 0
            else "final"
        ),
        "aux_partition_primary_metric_name": "eval_miou",
        "aux_partition_secondary_metric_name": "eval_ari",
        "headline_metric_note": (
            "upstream_eval_iou/dice follow the AutoSAM-style resized-binary MoNuSeg view: "
            "resize the original-resolution foreground probability map to upstream_eval_image_size with bilinear, "
            "resize the target mask with nearest, then threshold the resized prediction."
        ),
        ...
        "model_family": (
            "foreground_probe"
            if variant in GLAS_FROZEN_FEATURE_PROBE_VARIANT_SPECS
            else "coarse_plus_multibank_null_refine_mask_head"
            if is_multibank_null_refine_variant(variant)
            else "coarse_plus_cross_attention_refine_mask_head"
            if is_cross_attention_refine_variant(variant)
            else "coarse_plus_attention_refine_mask_head"
            if is_attention_refine_variant(variant)
            else "serialized_local_routed_refinement_mask_head"
            if is_serialized_local_routed_refine_variant(variant)
            else "serialized_progressive_refinement_mask_head"
            if is_serialized_refine_variant(variant)
            else "coarse_plus_fine_residual_mask_head"
            if is_coarse_plus_residual_variant(variant)
            else resolve_frozen_mask_head_head_family(variant)
            if is_memory_head_variant(variant)
        ...
        "head_kind": (
            ...
            if variant in GLAS_FROZEN_FEATURE_PROBE_VARIANT_SPECS
            else "coarse_plus_multibank_null_refine"
            if is_multibank_null_refine_variant(variant)
            else "coarse_plus_cross_attention_refine"
            if is_cross_attention_refine_variant(variant)
            else "coarse_plus_attention_refine"
            if is_attention_refine_variant(variant)
            else "serialized_local_routed_refinement"
            if is_serialized_local_routed_refine_variant(variant)
            else "serialized_progressive_refinement"
            if is_serialized_refine_variant(variant)
            else "coarse_plus_fine_residual"
            if is_coarse_plus_residual_variant(variant)
            else "memory_attention"
        ...
        flattened.update(
            {
                ...
                "coarse_upstream_eval_iou": coarse_mean_metrics["coarse_upstream_eval_iou"],
                "coarse_upstream_eval_dice": coarse_mean_metrics["coarse_upstream_eval_dice"],
                "final_minus_coarse_upstream_eval_iou": coarse_mean_metrics["final_minus_coarse_upstream_eval_iou"],
                "final_minus_coarse_upstream_eval_dice": coarse_mean_metrics["final_minus_coarse_upstream_eval_dice"],
            }
        )
...
def build_glas_frozen_mask_head_summary_markdown(summary: dict[str, Any]) -> str:
    lines = [
        "# MoNuSeg Frozen SAM Mask Head Summary",
        "",
        ...
        f"- Validation holdout count: `{summary.get('validation_holdout_count')}`",
        f"- Validation subset seed: `{summary.get('validation_subset_seed')}`",
        f"- Train subset manifest: `{summary.get('train_subset_manifest_output_path') or summary.get('train_subset_manifest_path')}`",
        f"- Headline metric source: `{summary.get('headline_metrics_source')}`",
        ...
        f"- Headline evaluation view: `{summary.get('headline_evaluation_view')}`",
        f"- Selection metric: `{summary.get('selection_metric')}` on split `{summary.get('selection_split')}` with checkpoint mode `{summary.get('selection_checkpoint_mode')}`",
        f"- Checkpoint selection materialized as: `{summary.get('checkpoint_selection_materialized')}`",
        f"- Mean upstream IoU: `{summary['mean_metrics']['upstream_eval_iou']:.6f}`",
        f"- Mean upstream Dice: `{summary['mean_metrics']['upstream_eval_dice']:.6f}`",
    ...
    lines.extend(
        [
            ...
            f"- Headline checkpoint: `{summary.get('headline_checkpoint_path')}` (`{summary.get('headline_checkpoint_role')}`)",
            f"- Final checkpoint: `{summary.get('final_checkpoint_path')}`",
            f"- Best epoch by eval Dice: `{summary.get('best_epoch_by_eval_dice')}`",
            f"- Best eval Dice: `{summary.get('best_eval_dice')}`",
            f"- Best epoch by selection metric: `{summary.get('best_epoch_by_selection_metric')}`",
            f"- Best selection metric value: `{summary.get('best_selection_metric_value')}`",
    ...
    *,
    ...
    sections = [
        ExperimentTermsSection(
            ...
            bullets=(
                f"Command family: `{getattr(args, 'command', 'monuseg-frozen-mask-head')}`.",
                f"Run kind: `{run_kind}`.",
                f"Variant: `{variant}`.",
                    f"Variant summary: {variant_spec['summary']}",
                f"Dataset: `{GLAS_DATASET_ID}` from Hugging Face dataset `{getattr(args, 'dataset_name', MONUSEG_HF_DATASET_NAME)}`.",
        ...
        ExperimentTermsSection(
            ...
            bullets=(
                ...
                (
                    f"Validation split: `val` derived from the official train split via `{describe_monuseg_validation_policy(holdout_count=resolve_mask_head_validation_holdout_count(args), subset_seed=resolve_mask_head_validation_subset_seed(args))}`."
                    if resolve_mask_head_validation_holdout_count(args) > 0
                    else "Validation is disabled by default; the full official train split is used unless an explicit holdout is requested."
                ),
                ...
                "The repo-native MoNuSeg loader uses the Hugging Face mirror and preserves the official challenge train split by default; holdout validation is explicit and opt-in.",
                (
                    ...
                    else "No train subset manifest is applied. The default SAM3 baseline contract uses the full official MoNuSeg train split unless a holdout is explicitly requested."
                ),
                (
                    f"Validation subset manifest: `{getattr(args, '_resolved_validation_subset_manifest_output_path', None)}`."
                    if resolve_mask_head_validation_holdout_count(args) > 0
                    else "Validation subset manifest: `none` (validation disabled by default)."
                ),
                "Foreground polarity contract: the nucleus mask is the positive class and corresponds to `texture_a_mask` in the MoNuSeg adapter.",
                ...
                (
                    ...
                    if resolve_mask_head_resize_hw(args) is not None
                        ...
                        f"Backbone preprocessing mode: `{resolve_mask_head_backbone_preprocessing_mode(args)}`. "
                        f"This mirrors AutoSAM-style `ResizeLongestSide({AUTOSAM_STYLE_BACKBONE_LONG_SIDE})` before frozen feature extraction while restoring predictions to original resolution for eval."
                        if resolve_mask_head_backbone_preprocessing_mode(args) == "autosam_style_1024"
                        else "No explicit resize is applied. Images and masks stay at native decoded MoNuSeg resolution."
                    )
                ),
            ),
        ),
        ...
        ExperimentTermsSection(
            ...
            bullets=(
                ...
                f"Checkpoint selection: `{resolve_mask_head_selection_checkpoint_mode(args)}` using `{resolve_mask_head_selection_metric(args)}` on split `{resolve_mask_head_selection_split(args)}`.",
                f"AutoSAM-style upstream eval image size: `{resolve_mask_head_upstream_eval_image_size(args)}`.",
                "Matching the AutoSAM metric view alone is not enough for backbone-input comparability; this run records the backbone preprocessing mode explicitly.",
                (
                ...
                "Primary supervised metrics: direct nucleus foreground IoU and Dice.",
                "AutoSAM-style headline metrics: `upstream_eval_iou` and `upstream_eval_dice` after resized-binary evaluation at the configured upstream eval image size.",
        ...
        ExperimentTermsSection(
            ...
            bullets=(
                f"Common run files: `config.json`, `experiment_terms.md`, `summary.json`, `summary.csv`, `summary.md`, `protocol.json`, `paper_row.json`, `fairness.md`, `benchmark_manifest.csv`, `per_sample_metrics.csv`, `per_sample_metrics.jsonl`, and `visuals_manifest.jsonl` under `{render_relative_path(GLAS_FROZEN_MASK_HEAD_OUTPUT_ROOT)}`.",
        ...
        ExperimentTermsSection(
            ...
            bullets=(
                "Hard-fail on missing or unreadable Hugging Face MoNuSeg assets, empty sample sets, or inconsistent SAM pyramid channels across samples.",
    ...
    return render_experiment_terms_markdown(
        title="MoNuSeg Frozen SAM Mask Head Experiment Terms",
        summary_lines=(
            "This file describes the exact frozen-feature supervised MoNuSeg run that produced this directory.",
        ...
        related_paths=(
            ...
            render_relative_path(Path("src/rwtd_sam3/eval/monuseg_frozen_feature_mask_head.py")),
        ),
    )
...
run_monuseg_frozen_mask_head_train = run_glas_frozen_mask_head_train
run_monuseg_frozen_mask_head_eval = run_glas_frozen_mask_head_eval
