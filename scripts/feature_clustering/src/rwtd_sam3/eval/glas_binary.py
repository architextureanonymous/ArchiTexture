"""GlaS adapter for registered cross-dataset SAM-feature experiments.

This module exposes the local GlaS image/mask pairs behind
``predict-glas-binary`` and ``eval-glas-binary``. It reuses the same
registered SAM-feature experiments used on RWTD, STLD, CAID, DeTexture
ADE20K, and CSTD, applies permutation-invariant gland-vs-background
scoring, and writes the standard run artifacts under ``outputs/glas_binary/``.
"""
...
GLAS_BINARY_DEFAULT_VARIANT = DEFAULT_CROSS_DATASET_EXPERIMENT
GLAS_BINARY_SUPPORTED_VARIANTS = CROSS_DATASET_EXPERIMENT_VARIANTS
GLAS_BINARY_OUTPUT_ROOT = Path("outputs") / "glas_binary"
GLAS_HARDWARE_COMPATIBILITY_STANDARD = "glas_streaming_eval_v1"
GLAS_HARDWARE_COMPATIBILITY_DESCRIPTION = (
    "GlaS evaluation must stream decoded image/mask pairs instead of preloading the dataset, "
    "must resolve the dataset root once per run, and must persist the active sample-loading mode into run artifacts."
)
GLAS_BINARY_SCALAR_FIELDS = (
    "eval_miou",
    "eval_ari",
    "miou",
    "ari",
...
class GlasBinarySampleResult:
    """Per-sample GlaS benchmark payload."""
...
def _compute_direct_foreground_view(
    *,
    prediction_a: np.ndarray,
    prediction_b: np.ndarray,
    foreground_mask: np.ndarray,
) -> dict[str, float | str]:
    """Score the same raw two-cluster output under a foreground-only orientation."""

    cluster_a_metrics = compute_binary_metrics(prediction_a, foreground_mask)
    cluster_b_metrics = compute_binary_metrics(prediction_b, foreground_mask)
    if cluster_b_metrics.iou > cluster_a_metrics.iou:
        chosen_metrics = cluster_b_metrics
        assignment = "cluster_b->foreground"
    else:
        chosen_metrics = cluster_a_metrics
        assignment = "cluster_a->foreground"
    return {
        "foreground_cluster_a_iou": cluster_a_metrics.iou,
        "foreground_cluster_a_dice": cluster_a_metrics.dice,
        "foreground_cluster_b_iou": cluster_b_metrics.iou,
        "foreground_cluster_b_dice": cluster_b_metrics.dice,
        "direct_foreground_iou": chosen_metrics.iou,
        "direct_foreground_dice": chosen_metrics.dice,
        "direct_foreground_assignment": assignment,
    }


def run_glas_binary_predict_one(args) -> dict[str, Any]:
    """Run one registered experiment on one GlaS sample."""

    sample = get_glas_binary_sample(dataset_root=args.dataset_root, split=args.split, index=args.index)
    output_dir = prepare_glas_binary_output_dir(
        args.output_dir,
    ...
    config_payload = build_glas_binary_run_config(
        args,
        overview=overview,
    ...
    write_text(
        ...
        build_glas_binary_experiment_terms_markdown(
            args,
            dataset_partition=None,
            selected_sample_count=1,
        ),
    ...
    evaluation = evaluate_glas_binary_sample(
        sample=sample,
        variant=args.variant,
        runner=runner,
    )
    ...
    if args.save_visuals:
        panel_path = save_glas_binary_panel(
        ...
        write_jsonl(
            ...
            [
                build_glas_binary_visual_record(
                    sample=sample,
                    evaluation=evaluation,
                    variant=args.variant,
...
def run_glas_binary_evaluation(args) -> dict[str, Any]:
    """Run one registered experiment over the local GlaS assets."""
    ...
    if num_total_samples < 1:
        raise RuntimeError("The GlaS root did not yield any samples to evaluate.")
    ...
    LOGGER.info(
        "Glas eval uses streamed sample decoding under hardware standard %s; limit=%s.",
        GLAS_HARDWARE_COMPATIBILITY_STANDARD,
        args.limit,
    )
    ...
    try:
        progress = tqdm(total=num_total_samples, desc="eval:glas", unit="sample")
        for step, sample in enumerate(
            ...
            if args.wandb:
                ...
                if step % args.log_every == 0:
                    preview_image = render_glas_binary_panel(
                        sample=sample,
                        evaluation=evaluation,
                        variant=args.variant,
                    )
                    caption = build_visual_caption(
                        sample=sample,
                        protocol=_glas_protocol_label(args.variant),
        ...
        if not rows:
            raise RuntimeError(
                "The GlaS benchmark produced no successful evaluations. "
        ...
        summary = build_glas_binary_summary(
            overview=overview,
            variant=args.variant,
            model_id=args.model_id,
            rows=rows,
        ...
        write_text(output_dir / "summary.md", build_glas_binary_markdown_summary(summary))
    finally:
...
def evaluate_glas_binary_sample(
    sample: GlasBinarySample,
    variant: str,
    runner,
) -> GlasBinarySampleResult:
    """Evaluate one registered experiment on one GlaS sample."""
    ...
    if variant == "boundary_refine_sweep":
        ...
        direct_foreground_view = _compute_direct_foreground_view(
            prediction_a=np.asarray(baseline_output.mask_a, dtype=bool),
            prediction_b=np.asarray(baseline_output.mask_b, dtype=bool),
            foreground_mask=sample.texture_a_mask,
        )
        ...
        row.update(direct_foreground_view)
        row.update(
            build_canonical_evaluation_fields(
                miou=assignment.chosen_miou,
                ari=assignment.chosen_ari,
...
def build_glas_binary_run_config(
    args,
    overview: GlasBinaryOverview | None,
    dataset_partition: DatasetPartitionSelection | None,
    selected_sample_count: int | None,
) -> dict[str, Any]:
    """Build a stable run-config payload for GlaS."""
    ...
    config = {
        ...
        "model_id": args.model_id,
        "device": args.device,
        "hardware_compatibility_standard": GLAS_HARDWARE_COMPATIBILITY_STANDARD,
        "hardware_compatibility_description": GLAS_HARDWARE_COMPATIBILITY_DESCRIPTION,
        ...
        "primary_metric_reason": (
            "GlaS exposes one gland mask per image, so the adapter evaluates the gland region and its "
            "complement under both cluster assignments and reports the better partition-invariant result."
        ),
        "versions": discovered_package_versions(),
    }
    if overview is not None:
        config["resolved_dataset_root"] = str(overview.dataset_root)
        config["resolved_data_dir"] = str(overview.data_dir)
        config["requested_split"] = overview.split
        config["num_examples"] = overview.num_examples
        config["evaluation_view"] = overview.evaluation_view
        config["grade_csv_path"] = str(overview.grade_csv_path) if overview.grade_csv_path is not None else None
    append_dataset_partition_fields(config, dataset_partition, selected_sample_count=selected_sample_count)
    return config


def build_glas_binary_experiment_terms_markdown(
    args,
    *,
    dataset_partition: DatasetPartitionSelection | None,
    selected_sample_count: int | None,
) -> str:
    """Render a dataset-specific protocol-spec document for GlaS."""

    spec = get_cross_dataset_experiment_spec(args.variant)
    lines = [
        "# GlaS Benchmark Terms",
        "",
        "## Run Scope",
        "",
        f"- Command family: `{getattr(args, 'command', 'glas-binary')}`",
        f"- Dataset id: `{GLAS_DATASET_ID}`",
        f"- Requested dataset root: `{args.dataset_root}`",
        "- Expected local root form: either a flat GlaS directory containing `train_*`, `testA_*`, `testB_*`, and `*_anno.bmp` files, or an extracted Warwick archive that contains such a directory in a descendant folder.",
        f"- Requested split: `{args.split}`",
        ...
        f"- Hardware compatibility standard: `{GLAS_HARDWARE_COMPATIBILITY_STANDARD}`",
        ...
        "## Dataset Semantics",
        "",
        "- Each sample is one RGB histology image plus one paired `*_anno.bmp` gland mask. The adapter defines texture A as the gland mask and texture B as its complement.",
        "- `train_*` images form the training split; `testA_*` and `testB_*` together form the test split; `all` concatenates both in natural order.",
        "- If `Grade.csv` is present, the adapter preserves the grade string in per-sample metadata but does not use it for scoring.",
        ...
        "2. The dataset adapter reuses the registered runner unchanged and applies only GlaS-specific GT decoding, permutation-invariant assignment, split handling, and output-directory handling.",
        ...
        "## Output Contract",
        "",
        "- `prediction.json` / `prediction.png` for `predict-glas-binary`.",
        "- `per_sample_metrics.csv`, `summary.json`, `summary.md`, and `visuals_manifest.jsonl` for `eval-glas-binary`.",
        f"- The preview contract comes from the registry entry for `{args.variant}` and is currently `{spec.visual_contract}`.",
        "",
        "## Failure Semantics",
        "",
        "- There is no silent fallback to another dataset, split, or refinement stage.",
        "- Missing dataset roots, unmatched image/mask pairs, empty gland or empty background regions, shape mismatches, or missing official Meta `sam3` features are explicit failures.",
...
def build_glas_binary_summary(
    ...
    """Aggregate one local GlaS run."""

    spec = get_cross_dataset_experiment_spec(variant)
    mean_metrics = {name: mean(float(row[name]) for row in rows) for name in GLAS_BINARY_SCALAR_FIELDS}
    median_metrics = {name: median(float(row[name]) for row in rows) for name in GLAS_BINARY_SCALAR_FIELDS}
    summary = {
        "dataset_id": GLAS_DATASET_ID,
        "dataset_root": str(overview.dataset_root),
        "data_dir": str(overview.data_dir),
        "split": overview.split,
        ...
        "primary_metric_reason": (
            "GlaS exposes one gland mask per image, so this run scores the gland region and its complement "
...
def build_glas_binary_markdown_summary(summary: dict[str, Any]) -> str:
    """Render a concise Markdown summary for GlaS."""
    ...
    lines = [
        ...
        f"- Resolved data dir: `{summary['data_dir']}`",
    ...
    for metric_name in GLAS_BINARY_SCALAR_FIELDS:
        lines.append(
...
def save_glas_binary_panel(
    output_path: str | Path,
    sample: GlasBinarySample,
    evaluation: GlasBinarySampleResult,
    variant: str,
) -> Path:
    """Save the registered preview panel used for GlaS."""
    ...
    if evaluation.boundary_refine_sweep is not None:
        return save_boundary_refine_sweep_panel(
            ...
            protocol=_glas_protocol_label(variant),
            metric_summary=evaluation.metric_summary,
        )
...
def build_glas_binary_visual_record(
    sample: GlasBinarySample,
    evaluation: GlasBinarySampleResult,
    variant: str,
    visual_path: Path,
) -> dict[str, Any]:
    """Build one machine-readable visual record for GlaS."""
...
def resolve_glas_binary_output_dir(output_dir: str | None, variant: str) -> Path:
    """Resolve the output directory for a GlaS run."""

    if output_dir:
        requested_dir = Path(output_dir)
        if _normalize_output_path(requested_dir) == _normalize_output_path(GLAS_BINARY_OUTPUT_ROOT):
            safe_dir = requested_dir / variant
            LOGGER.warning(
                "Requested shared GlaS output root %s; redirecting this run to %s to avoid mixed artifacts.",
                requested_dir,
                safe_dir,
            )
            return safe_dir
    ...
    return GLAS_BINARY_OUTPUT_ROOT / f"glas-{variant}-{timestamp}"


def prepare_glas_binary_output_dir(
    output_dir: str | None,
    *,
    variant: str,
    run_kind: str,
    ...
    """Resolve and validate a GlaS output directory."""

    if run_kind not in {"predict", "eval"}:
        raise ValueError(f"Unsupported run_kind '{run_kind}'. Expected 'predict' or 'eval'.")
    resolved_dir = resolve_glas_binary_output_dir(output_dir, variant=variant)
    if output_dir and _normalize_output_path(Path(output_dir)) == _normalize_output_path(GLAS_BINARY_OUTPUT_ROOT):
        if run_kind == "predict":
            resolved_dir = resolved_dir.with_name(f"{variant}_index{sample_index}")
        _validate_glas_binary_output_dir(resolved_dir, requested_variant=variant, run_kind=run_kind)
        return resolved_dir
    _validate_glas_binary_output_dir(resolved_dir, requested_variant=variant, run_kind=run_kind)
    return resolved_dir


def _glas_protocol_label(variant: str) -> str:
    return f"glas_binary:{variant}"
...
def _validate_glas_binary_output_dir(
    output_dir: Path,
    *,
    ...
    for metadata_path in (output_dir / "config.json", output_dir / "summary.json", output_dir / "prediction.json"):
        ...
        if isinstance(existing_variant, str) and existing_variant != requested_variant:
            raise RuntimeError(
                f"Refusing to reuse Glas output directory '{output_dir}' because it belongs to variant "
